...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof itercumin, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'if an iterated value is a non-numeric value, the computed minimum value is `NaN`', function test( t ) {
	...
	for ( i = 0; i < values.length; i++ ) {
		v = it.next();
		if ( isnan( expected[ i ] ) ) {
			t.strictEqual( isnan( v.value ), true, 'returns expected value for iteration '+i );
		} else {
			t.strictEqual( v.value, expected[ i ], 'returns expected value for iteration '+i );
		}
	}
